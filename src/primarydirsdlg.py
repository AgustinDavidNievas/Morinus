...
#		self.placidianutprb.Enable(False)
	...
	def onPlacidian(self, event):
		...
		if not mun:
			if self.szbothrb.GetValue():
				bianchini = True
		if mun or self.bothrb.GetValue():
	...
	def enableZods(self, val):
		...
		self.promfixstarsckb.Enable(val)
		self.sigsyzckb.Enable(val)

		if val:
			if self.promfixstarsckb.GetValue():
				self.btnFixStars.Enable(True)
			else:
				self.btnFixStars.Enable(False)
		else:
			self.btnFixStars.Enable(False)
	...
	def onFixStarsCheck(self, event):
		self.btnFixStars.Enable(self.promfixstarsckb.GetValue())
	...
	def onSignificatorsDes(self, event):
		...
		self.sighousecuspsckb.SetValue(False)

		ckbs = [self.sigsunckb, self.sigmoonckb, self.sigmercuryckb, self.sigvenusckb, self.sigmarsckb, self.sigjupiterckb, self.sigsaturnckb, self.siguranusckb, self.signeptuneckb, self.sigplutockb, self.siganodeckb, self.sigdnodeckb, self.siglofckb, self.sigsyzckb]
		for i in range(len(ckbs)):
			ckbs[i].SetValue(False)
	...
	def onSignificatorsSel(self, event):
		...
		self.sighousecuspsckb.SetValue(True)
	...
	def onPromissorsDes(self, event):
		ckbs = [self.promsunckb, self.prommoonckb, self.prommercuryckb, self.promvenusckb, self.prommarsckb, self.promjupiterckb, self.promsaturnckb, self.promuranusckb, self.promneptuneckb, self.promplutockb, self.promanodeckb, self.promdnodeckb, self.promantsckb, self.prommidpointsckb, self.promlofckb, self.promtermsckb, self.promfixstarsckb]
		for i in range(len(ckbs)):
			ckbs[i].SetValue(False)

		self.btnFixStars.Enable(False)
		self.secmotionckb.Enable(False)
		self.secmotionitercb.Enable(False)


	def onPromissorsSel(self, event):
		ckbs = [self.promsunckb, self.prommoonckb, self.prommercuryckb, self.promvenusckb, self.prommarsckb, self.promjupiterckb, self.promsaturnckb, self.promuranusckb, self.promneptuneckb, self.promplutockb, self.promanodeckb, self.promdnodeckb, self.promantsckb, self.prommidpointsckb, self.promlofckb, self.promtermsckb, self.promfixstarsckb]
		for i in range(len(ckbs)):
			ckbs[i].SetValue(True)

		if self.bothrb.GetValue() or self.zodiacalrb.GetValue():
			self.btnFixStars.Enable(True)

		self.secmotionckb.Enable(True)
		self.secmotionitercb.Enable(True)


	def onAspectsDes(self, event):
	...
	def fill(self, options):
		self.promfixstarsckb.SetValue(options.pdfixstars)
		...
		if options.primarydir == primdirs.PrimDirs.PLACIDIANSEMIARC:
			...
			self.campanianrb.SetValue(True)
	...
	def check(self, options):
		...
		if self.placidiansemiarcrb.GetValue():
			...
			if options.primarydir != primdirs.PrimDirs.REGIOMONTAN:
				...
				changed = True
		elif self.campanianrb.GetValue():
			if options.primarydir != primdirs.PrimDirs.CAMPANIAN:
				options.primarydir = primdirs.PrimDirs.CAMPANIAN
				changed = True
		...
		if self.promfixstarsckb.GetValue() != options.pdfixstars:
			options.pdfixstars = self.promfixstarsckb.GetValue()
			changed = True

		num = len(self.options.pdfixstarssel)
		for i in range(num):
			if self.fsselections[i] != self.options.pdfixstarssel[i]:
				self.options.pdfixstarssel[i] = self.fsselections[i]
				changed = True
