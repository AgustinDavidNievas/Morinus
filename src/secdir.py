	...
	def compute(self):
		...
		if self.soltime:
			...
			if self.chart.time.bc:
				yt = -y
			jdmean = astrology.julday(yt, m, d, meantime, calflag)

			#Get jdapp
			ret, te, serr = astrology.time_equ(jdmean)
			jdapp = jdmean-te
			y, m, d, hr = astrology.revjul(jdapp, calflag)
...
#			print '%d:%02d:%02d' % (hour,minute,second)
		...
		if self.soltime:
			...
			if self.chart.time.cal == chart.Time.JULIAN:
				calflag = astrology.SE_JUL_CAL
			jdapp = astrology.julday(yt, m, d, hr, calflag)

			ret, te, serr = astrology.time_equ(jdapp)
			jdmean = jdapp+te
			y, m, d, hr = astrology.revjul(jdmean, calflag)
			hour,minute,second = util.decToDeg(hr)
#			print '%d:%02d:%02d' % (hour,minute,second)
